    ...
    <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden flex flex-col h-full">
      <div className="px-4 py-3 border-b border-zinc-200 flex items-center justify-between bg-slate-50">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-zinc-700 uppercase tracking-wider">Opportunity Ranking Engine</h3>
        </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        <div className="space-y-4">
          {opportunities.length ? opportunities.map((opp) => (
            <div key={opp.pair} className="p-4 rounded-lg border border-zinc-200/80 bg-slate-50 hover:bg-zinc-200/40 transition-colors">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-6 h-6 rounded bg-zinc-200 text-xs font-mono text-zinc-400 border border-zinc-300">
                    #{opp.rank}
                  </div>
                  <h4 className="text-lg font-bold text-zinc-900">{opp.pair}</h4>
                ...
                <div className="text-right">
                  <div className="text-2xl font-mono text-zinc-900">{opp.compositeScore}<span className="text-sm text-zinc-500">%</span></div>
              ...
              {opp.correlations.length > 0 && (
                <div className="mt-4 pt-3 border-t border-zinc-200 flex items-center gap-2">
                  ...
                  {opp.correlations.map(corr => (
                    <span key={corr.asset} className="text-xs text-zinc-600 bg-zinc-200 px-2 py-1 rounded">
...
function MetricCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string | number }) {
  return (
    <div className="bg-slate-50 rounded border border-zinc-200 p-2 flex flex-col gap-1">
      ...
      <div className="text-sm font-medium text-zinc-700 truncate">{value}</div>
    </div>
  );
}
